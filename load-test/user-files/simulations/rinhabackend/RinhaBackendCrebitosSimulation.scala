  ...
  setUp(
    validacaoTransacoesConcorrentes("d").inject(
      ...
      criterioClienteNaoEcontrado.inject(
        ...
        debitos.inject(
          rampUsersPerSec(numRampUp).to(numClientes).during(2.minutes),
          constantUsersPerSec(numClientes).during(2.minutes)
        ),
        creditos.inject(
          rampUsersPerSec(numRampUp).to(numClientes/2).during(2.minutes),
          constantUsersPerSec(numClientes/2).during(2.minutes)
        ),
        extratos.inject(
          rampUsersPerSec(numRampUp).to(numClientes/20).during(2.minutes),
          constantUsersPerSec(numClientes/20).during(2.minutes)
        )
      )
    )
